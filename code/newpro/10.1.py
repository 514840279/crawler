...
def read_detial(url):
    ...
    files = File_file()
    names = url.split('/')
    file_name = names[len(names)-1] +'.html'

    files.save_source(path=path,file=file_name, all_the_text=detial_html , encoding_='gb2312')
    colum=[
        ('title','//div[@class="newsTitle"]//text()','l'),
        ('pushDate', '//div[@class="newsfrom"]//text()', 'l'),
        ('content','//div[@class="newsContent"]//text()','sarra',',')
   ]
    ...
    csv = Csv_base()
    csv.write_csv_file_line(file_path=path + "/data.csv", str=['国防科技信息网', url, result[0][1], result[1][1], result[2][1]])
...
def main():
    ...
    csv = Csv_base()
    csv.write_csv_file_line(file_path=path+"/data.csv",mode='w+',str=['网站名','网址','标题','新闻内容','发布时间'])
    # 爬虫
    start_url = "http://www.dsti.net/Information/HyeList/spaceflight/%d"
    for i in range(1,729):
        url = start_url%(i)
        #print(url)
        list_html = htmlSource.get_html(url_p=url,type_p='rg')
        #print(list_html)
        colum=[('a','//div[@class="listMidContent"]//ul//li//a//@href','sab','http://www.dsti.net')]
