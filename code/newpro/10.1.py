...
def read_detial(url,i):
    ...
    csv = Csv_base()
    csv.write_csv_file_line(file_path=path + "/data.csv", str=['国防科技信息网', url, result[0][1], result[1][1], result[2][1],i,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))])
