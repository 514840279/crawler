...
class ResultData:
    ...
    def resultRefulence(self, rule_uuid, result, type):
        # 拼接查询映射关系表语句
        sql = "SELECT distinct  `table_uuid`FROM `application`.`sys_seed_result_ruler_info`WHERE ruler_uuid= '%s' " % (
            rule_uuid)
...
class SysCrawlerTaskInfo:
    ...
    def updateDelete(self,id,val):
        sql = " update sys_crawler_task_info set delete_flag = %d where uuid='%s'" %(val,id)
