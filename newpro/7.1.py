...
def read_detial(url,i):
    ...
    colum=[
        ('title','//div[@class="newscontent"]//h1[@class="news_title"]//text()','l'),
        ('pushDate', '//div[@class="news_about"]//p//text()', 'arr', 1),
        ('content','//div[@class="news_txt"]//text()','sarra',',')
   ]
    ...
    csv = Csv_base()
    csv.write_csv_file_line(file_path=path + "/data.csv", str=['澎湃防务', url, result[0][1], result[1][1], result[2][1],i,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))])
